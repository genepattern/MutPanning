/************************************************************           
 * MutPanning - Step 11_AlternativeModel					*
 * 															*   
 ...
 *															*   
 * Summary: This script is an alternative to the main 		*
 * script for the computation of the significance. 			*
 * This script is usde  for very very low mutation counts 	*
 * when almost all synonymous mutation counts are around	*
 * 0. Then the CBASE model does not have enough data to		*
 * estimate the background distribution of synonymous		*
 * mutations correctly and few nonsynonymous mutation 		*
 * counts in long genes can lead to false positive findings.*
 * In order to obtain a more conservative estimation of		*
 * significance, this script assumes that the nonsynonymous	*
 * mutations are purely distributed based on the gene		*
 * length, which allows the occurrence of nonsynonymous 	*
 * mutations in long genes, even when there are no syn		*
 * mutations. This script is executed in addition to the 	*
 * standard model if the sample size is very low or the		*
 * number of mutations are low. In this scenario this script*
 * might miss some genes. However, it also reduces the no	*
 * false positive genes, so that both outputs should be		*
 * compared. Apart from this difference how the total 		*
 * mutation counts are statistically handled all further	*
 * computational steps are the same.						*
 *************************************************************/
...
public class ComputeSignificance_Uniform {
	...
	* argument2: path to the hg19 folder
	...
	public static void main(String[] args){
		...
		try{
			...
			int non_syn_count=0;
			double non_syn_coverage=0;
			for (int i=0;i<genes.length;i++){
				for (int j=0;j<genes[i].size();j++){
					non_syn_count+=genes[i].get(j).count;
					non_syn_coverage+=genes[i].get(j).cov;
				}
			}
			for (int i=0;i<genes.length;i++){
				for (int j=0;j<genes[i].size();j++){
					if(genes[i].get(j).cov==0){
						genes[i].get(j).prob_nonsyn.add(1.0);
						continue;
					}
					double prob=1;
					int n=0;
					BinomialDistribution dist=new BinomialDistribution(non_syn_count,genes[i].get(j).cov/non_syn_coverage);
					while(prob>=Math.pow(10, -6)||n<=genes[i].get(j).count){
						prob=dist.probability(n);
						genes[i].get(j).prob_nonsyn.add(prob);
						n++;
					}
				}
			}
			...
			for (int i=0;i<5;i++){
			//	genes_all.get(i).sign_complete/=100.0;
			}
			...
			/*
			Comparator<Gene> comp_gene_final=(Gene g1, Gene g2)->{
				if(g1.fdr!=g2.fdr){
					return new Double(g1.fdr).compareTo(new Double(g2.fdr));
				}
				return -new Double(g1.count).compareTo(new Double(g2.count));
			};
			Collections.sort(genes_all,comp_gene_final);
			*/
	...
	private static class Subthread_pre extends Thread{
		volatile boolean done=false;
		int c=-1;
		
		public void run(){
			System.out.println("start pre"+chr[c]);
			try{
				...
				for (int nn=0;nn<genes[c].size();nn++){
					...
					genes[c].get(nn).count=sum(count2);
					genes[c].get(nn).count_syn=sum(count2_syn);
					genes[c].get(nn).cov=sum_double(coverage2);
					genes[c].get(nn).cov_syn=sum_double(coverage2_syn);
...
//			System.out.println("s3 done");
		...
		public double sign_longREVISED(int count_ext, ArrayList<Double> lambda2, ArrayList<Integer> count2, ArrayList<Double> probability, int err){//int err, 
			...
			if(lambda2.size()==0||NN<=1){
				return 1;
			}
			if(probability.size()<=NN){
				System.out.println(NN+"	"+count_ext);
				System.out.println("Probability too short");
				//System.exit(0);
				return 0;
			}
		...
		public void run(){
			...
			try{
				...
				for (int nn=0;nn<genes[c].size();nn++){
					...
					int err=3;//TODO: 3 ??
					//System.out.println(genes[c].get(nn).cov);
